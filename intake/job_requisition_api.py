...
async def delete_job(
    ...
    application_ids_result = await db.execute(select(Application.id).where(Application.job_id == job_id))
    application_ids = [row[0] for row in application_ids_result.all()]

    candidate_ids_result = await db.execute(select(Candidate.id).where(Candidate.job_id == job_id))
    candidate_ids = [row[0] for row in candidate_ids_result.all()]

    offer_ids = []
    if application_ids:
        offer_ids_result = await db.execute(select(Offer.id).where(Offer.application_id.in_(application_ids)))
        offer_ids = [row[0] for row in offer_ids_result.all()]

    session_ids = []
    chatbot_session_ids_result = await db.execute(select(ChatbotSession.session_id).where(ChatbotSession.job_id == job_id))
    session_ids = [row[0] for row in chatbot_session_ids_result.all()]

    if session_ids:
        await db.execute(delete(ChatbotAnswer).where(ChatbotAnswer.session_id.in_(session_ids)))
        await db.execute(delete(ChatbotSession).where(ChatbotSession.session_id.in_(session_ids)))

    if application_ids:
        await db.execute(delete(Interview).where(Interview.application_id.in_(application_ids)))

    if offer_ids:
        await db.execute(delete(OnboardingTask).where(OnboardingTask.offer_id.in_(offer_ids)))
        await db.execute(delete(Onboarding).where(Onboarding.offer_id.in_(offer_ids)))
    ...
    await db.execute(delete(Communication).where(Communication.job_id == job_id))
    await db.execute(delete(JobPosting).where(JobPosting.job_id == job_id))
    if candidate_ids:
    ...
    return {
        ...
        "deleted": {
            ...
            "chatbot_sessions": len(session_ids),
        },
    }
