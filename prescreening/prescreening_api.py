...
async def get_prescreening_stats(job_id: Optional[str] = None, db: AsyncSession = Depends(get_db)):
    ...
    from shared.db.models import ChatbotSession, ChatbotAnswer, Onboarding
    ...
    total_in_prescreening = len([s for s in sessions if s.status == "IN_PROGRESS"])
    ...
    for s in sessions:
        answers_result = await db.execute(
            select(ChatbotAnswer).where(ChatbotAnswer.session_id == s.session_id)
        )
    ...
    bgv_query = select(Onboarding)
    bgv_result = await db.execute(bgv_query)
    onboardings = bgv_result.scalars().all()
    
    bgv_cleared = len([o for o in onboardings if o.bgv_status == "CLEARED"])
    bgv_pending = len([o for o in onboardings if o.bgv_status == "PENDING"])
    
    return PrescreeningStats(
        total_in_prescreening=total_in_prescreening,
...
# ── CANDIDATE FACE CHATBOT ENDPOINTS ──
...
class CandidateCompleteRequest(BaseModel):
    token: str


async def _get_valid_session_async(token: str, db: AsyncSession) -> ChatbotSession:
    result = await db.execute(select(ChatbotSession).where(ChatbotSession.token == token).limit(1))
    ...
    # Check expiration
    if session.expires_at and datetime.utcnow() > session.expires_at.replace(tzinfo=None):
        session.status = "EXPIRED"
        await db.commit()
...
@router.post("/start")
async def start_candidate_session(req: CandidateStartRequest, db: AsyncSession = Depends(get_db)):
    session = await _get_valid_session_async(req.token, db)
    
    # Get candidate info
    cand_res = await db.execute(select(Candidate).where(Candidate.id == session.candidate_id).limit(1))
    candidate = cand_res.scalar_one_or_none()
    if not candidate:
        raise HTTPException(status_code=404, detail="Candidate not found")
        
    # Get job info
    job_res = await db.execute(select(Job).where(Job.id == session.job_id).limit(1))
    job = job_res.scalar_one_or_none()
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
        
    # Load or generate questions if empty
    questions = []
    if session.questions:
        try:
            questions = json.loads(session.questions)
    ...
    if not questions:
        # Generate dynamically using Groq
        questions = _generate_questions_groq(job.title, job.description or "")
        session.questions = json.dumps(questions)
        session.status = "IN_PROGRESS"
        await db.commit()
...
@router.post("/answer")
async def submit_candidate_answer(req: CandidateAnswerRequest, db: AsyncSession = Depends(get_db)):
    session = await _get_valid_session_async(req.token, db)
    ...
    ans_res = await db.execute(
        select(ChatbotAnswer).where(
            ChatbotAnswer.session_id == session.session_id,
            ChatbotAnswer.question_index == req.question_index
        ).limit(1)
    )
...
async def complete_candidate_session(req: CandidateCompleteRequest, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ChatbotSession).where(ChatbotSession.token == req.token).limit(1))
    session = result.scalar_one_or_none()
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    # Evaluate answers synchronously using answer_evaluator
    from prescreening.answer_evaluator import evaluate_session
    try:
        evaluate_session(session.session_id)
    except Exception as e:
        logger.error(f"Sync evaluation failed: {e}")
...
@router.get("/candidates/{candidate_id}/session")
async def get_session_by_candidate(candidate_id: str, db: AsyncSession = Depends(get_db)):
    """Fetch prescreening session details for a specific candidate (recruiter dashboard)."""
    session_result = await db.execute(
        select(ChatbotSession).where(ChatbotSession.candidate_id == candidate_id).order_by(ChatbotSession.created_at.desc()).limit(1)
    )
    session = session_result.scalar_one_or_none()
    if not session:
        return {"success": False, "exists": False}
        
    candidate_result = await db.execute(select(Candidate).where(Candidate.id == session.candidate_id).limit(1))
    candidate = candidate_result.scalar_one_or_none()
    
    job_result = await db.execute(select(Job).where(Job.id == session.job_id).limit(1))
    job = job_result.scalar_one_or_none()
    
    # Calculate answered questions count
    from shared.db.models import ChatbotAnswer
    ...
    return {
        ...
        "assessment_url": f"http://localhost:8000/prescreening.html?session_id={session.session_id}",
        "completed_at": session.completed_at.isoformat() if session.completed_at else None,
...
async def get_session_details(session_id: str, db: AsyncSession = Depends(get_db)):
    """Fetch session details (questions, job title, candidate name) for candidate portal."""
    session_result = await db.execute(
        select(ChatbotSession).where(ChatbotSession.session_id == session_id).limit(1)
    )
    session = session_result.scalar_one_or_none()
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    # Check expiration
    if session.expires_at and session.expires_at < datetime.utcnow() and session.status == "IN_PROGRESS":
        session.status = "EXPIRED"
        await db.commit()
    ...
    if session.questions:
        try:
            ...
            questions = [{ "id": i + 1, "question": q, "minChars": 20 } for i, q in enumerate(questions_list)]
        except Exception:
            pass
...
async def get_session_results(session_id: str, db: AsyncSession = Depends(get_db)):
    """Fetch submitted answers and AI evaluation details for recruiter review."""
    session_result = await db.execute(
        select(ChatbotSession).where(ChatbotSession.session_id == session_id).limit(1)
    )
    session = session_result.scalar_one_or_none()
    ...
    answers_result = await db.execute(
        select(ChatbotAnswer).where(ChatbotAnswer.session_id == session_id).order_by(ChatbotAnswer.question_index)
    )
    answers = answers_result.scalars().all()
    ...
    feedback = ""
    if answers:
        feedback_list = [ans.reason for ans in answers if ans.reason]
        if feedback_list:
    ...
    return {
        ...
        "answers": [{
            ...
            "question": ans.question,
            "answer": ans.answer,
            "ai_score": ans.ai_score or "Not scored",
            "ai_feedback": ans.reason or "No feedback"
        } for ans in answers],
        "evaluation": {
            "score": round(avg_score, 1),
            "verdict": verdict,
            "feedback": feedback or "AI evaluation complete"
        }
    }
