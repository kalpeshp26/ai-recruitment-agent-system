...
async def resend_outreach(request: OutreachRequest, db: AsyncSession = Depends(get_db)):
    """Resend outreach / prescreening invitation email for a candidate."""
    # Verify candidate and job exist
    ...
    # Check if a prescreening session already exists
    result = await db.execute(
        select(ChatbotSession).where(
            ChatbotSession.candidate_id == request.candidate_id,
            ChatbotSession.job_id == request.job_id
        )
    )
    session = result.scalar_one_or_none()
    
    if not session:
        # If no session exists, start the standard outreach session creation
        await auto_create_prescreening_session(request.candidate_id, request.job_id, db)
        return {"message": "Outreach session created and email sent"}
        
    # If session exists, reuse its token to build the URL and resend the email
    screening_url = f"{SCREENING_BASE_URL}?token={session.token}"
    
    # Resend invitation email
    await send_prescreening_invitation(candidate, job, screening_url, db)
    
    # Update application status to PRESCREENING if not already
    result = await db.execute(
    ...
    if app and app.status not in ["OUTREACH_SENT", "PRESCREENING"]:
        app.status = "PRESCREENING"
        await db.commit()
        
    return {
        "message": "Outreach email resent successfully",
        "candidate_id": request.candidate_id,
        "job_id": request.job_id
    }
