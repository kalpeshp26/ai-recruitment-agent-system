...
async def submit_response(
    ...
    if not force_next:
        ...
        content_score = classifier_result["content_score"] * 100.0
    ...
    intent_score = intent_score_map.get(intent, 60.0)
...
async def get_report(
    ...
    for mt in main_turns:
        ...
        turn_reviews.append(TurnReviewItem(
            ...
            content_score=mt.content_score * 100.0 if mt.content_score is not None and mt.content_score <= 1.0 else mt.content_score,
            intent=mt.intent,
            behavior_score=round((0.5 * eye_contact + 0.5 * head_stability) * 100.0, 1),
            final_score=mt.final_score * 100.0 if mt.final_score is not None and mt.final_score <= 1.0 else mt.final_score,
    ...
    content_scores = [s * 100.0 if s <= 1.0 else s for s in content_scores]
    final_scores = [s * 100.0 if s <= 1.0 else s for s in final_scores]
    ...
    for t in all_turns:
        ...
        behavior_scores.append((0.5 * ec + 0.5 * hs) * 100.0)
