...
from config import GROQ_API_KEY, REDIS_URL
from shared.db.database import get_db_sync
from shared.db.interview import (
    InterviewSession,
...
from interview.services.interview_rl_engine import InterviewRLEngine

# Temporary auth bypass (TODO: integrate with root system auth)
class DummyUser:
    id = 1
    email = "admin@system.com"

def get_current_user():
    """Temporary auth bypass - returns dummy user"""
    return DummyUser()
...
async def start_interview(
    ...
    # Check if a session already exists for this session_id (e.g. created by recruiter workflow)
    ...
    if interview:
        ...
        interview.total_turns = 10
        interview.rl_state = rl_engine.to_dict()
        interview.status = "in_progress"
        # Ensure candidate_id and job_id are populated if not already
        if not interview.candidate_id or not interview.job_id:
            ...
            if placeholder:
                interview.candidate_id = placeholder[0]
                interview.job_id = placeholder[1]
    else:
        from sqlalchemy import text
        placeholder = db.execute(text(
            "SELECT candidate_id, job_id FROM interview_sessions WHERE session_id = :sid AND candidate_id IS NOT NULL LIMIT 1"
        ...
        cand_id = placeholder[0] if placeholder else None
        j_id = placeholder[1] if placeholder else None
...
async def submit_response(
    ...
    if action == "COMPLETE":
        ...
        # Publish interview completion event for Stage 8 (Offer Management)
        ...
        if turns:
            avg_content = sum(t.content_score for t in turns) / len(turns)
            avg_behavior = sum(t.behavior_score for t in turns) / len(turns)
            ...
            print(f"[Interview] ✅ Published interview.completed event for candidate {interview.candidate_id}")
