...
class InterviewSession(Base):
    ...
    session_id: int = Column(Integer, ForeignKey("assessment_sessions.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    rl_state: dict = Column(JSONB, server_default=text("'{}'::jsonb"), nullable=False)
...
class ApprovedQuestionPool(Base):
    ...
    id: int = Column(Integer, primary_key=True, index=True)
    session_id: int = Column(Integer, ForeignKey("assessment_sessions.id", ondelete="CASCADE"), nullable=False, index=True)
    extracted_skills: list = Column(JSONB, server_default=text("'[]'::jsonb"), nullable=False)
    extracted_projects: dict = Column(JSONB, server_default=text("'{}'::jsonb"), nullable=False)
    question_pool: list = Column(JSONB, nullable=False)
...
class InterviewTurn(Base):
    ...
    behavioral_snapshot: dict = Column(JSONB, server_default=text("'{}'::jsonb"), nullable=False)
