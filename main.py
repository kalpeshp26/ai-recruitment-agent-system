...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        event_bus.subscribe("interview.completed", process_interview_completed_event)
        print("SUCCESS: Session manager subscribed to interview.completed events")
    except Exception as e:
        print(f"WARNING: Could not subscribe to interview.completed events: {e}")
...
# Serve frontend assets and support SPA fallback at root paths
@app.get("/{full_path:path}", include_in_schema=False)
async def spa_fallback(full_path: str):
    """Serve a static file from `frontend/` when it exists, otherwise return index.html.
    This makes root-relative asset paths like `/style.css` resolve correctly.
    """
    try:
        candidate = FRONTEND_DIR / full_path
        # Prevent escaping outside frontend dir
        if candidate.exists() and candidate.is_file() and str(candidate.resolve()).startswith(str(FRONTEND_DIR.resolve())):
